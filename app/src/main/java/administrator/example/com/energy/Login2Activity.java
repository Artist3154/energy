            ...
            public void afterEvent(int event, int result, Object data) {
                ...
                if (result == SMSSDK.RESULT_COMPLETE) { //回调完成
                    ...
                    if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) { //提交验证码成功
                        ...
                        startActivity(new Intent(Login2Activity.this, MainActivity.class)); //页面跳转
...
//                SMSSDK.getSupportedCountries();//获取短信目前支持的国家列表
                if(!userName.getText().toString().trim().equals("")){
                    if (checkTel(userName.getText().toString().trim())) {
                        ...
                        Toast.makeText(Login2Activity.this, "请输入正确的手机号码", Toast.LENGTH_SHORT).show();
                    }
                }else{
                ...
                if (!userName.getText().toString().trim().equals("")) {
                    if (checkTel(userName.getText().toString().trim())) {
                        if (!validateNum.getText().toString().trim().equals("")) {
                            ...
                            Toast.makeText(Login2Activity.this, "请输入验证码", Toast.LENGTH_SHORT).show();
                        }
                    }else{
                        Toast.makeText(Login2Activity.this, "请输入正确的手机号码", Toast.LENGTH_SHORT).show();
                    }
